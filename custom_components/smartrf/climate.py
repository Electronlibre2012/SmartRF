...
class SmartIRClimate(ClimateEntity, RestoreEntity):
    ...
    async def send_command(self):
        async with self._temp_lock:
            try:
                self._on_by_remote = False
                operation_mode = self._hvac_mode.lower() if self._hvac_mode else None
                fan_mode = self._current_fan_mode.lower() if self._current_fan_mode else None
                swing_mode = self._current_swing_mode.lower() if self._current_swing_mode else None
                ...
                if operation_mode == HVACMode.OFF:
                    if 'off' in self._commands:
                        ...
                        _LOGGER.warning("No 'off' command defined in JSON")
                    return

                # Envoie une commande 'on' si disponible
                if 'on' in self._commands:
                    await self._controller.send(self._commands['on'])
                    await asyncio.sleep(self._delay)

                # Construction des combinaisons de clés à tester
                possible_keys = []

                if self._support_swing and swing_mode:
                    possible_keys.extend([
                        f"{operation_mode}_{fan_mode}_{swing_mode}_{target_temperature}",
                        f"{operation_mode}_{fan_mode}_{swing_mode}",
                    ])

                possible_keys.extend([
                    f"{operation_mode}_{fan_mode}_{target_temperature}",
                    f"{operation_mode}_{target_temperature}",
                    f"{operation_mode}_{fan_mode}",
                    operation_mode,
                    fan_mode,
                    target_temperature
                ])
                ...
                for key in possible_keys:
                    if key in self._commands:
                        _LOGGER.debug(f"Sending command with key: {key}")
                        await self._controller.send(self._commands[key])
                        return

                _LOGGER.warning(
                    f"No matching command found for mode='{operation_mode}', "
                    f"fan='{fan_mode}', temp='{target_temperature}', swing='{swing_mode}'"
                )

            except Exception as e:
                _LOGGER.exception("Error sending RF command: %s", e)
